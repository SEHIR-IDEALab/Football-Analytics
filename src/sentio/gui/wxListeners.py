...
class wxListeners:
    ...
    def on_open_plot(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if Parameters.IS_VORONOI_DIAGRAM_ON:
                self.wx_gui.voronoi.update(self.wx_gui.filterTeamPlayers(self.wx_gui.visual_idToPlayers))

            if Parameters.IS_DOMINANT_REGION_ON:
                self.wx_gui.dominant_region.update(self.wx_gui.filterTeamPlayers(self.wx_gui.visual_idToPlayers))
    ...
    def on_dominant_region(self, e):
        Parameters.IS_DOMINANT_REGION_ON = not Parameters.IS_DOMINANT_REGION_ON
        print "dominant region: ", Parameters.IS_DOMINANT_REGION_ON

        if Parameters.IS_DOMINANT_REGION_ON:
            self.wx_gui.dominant_region.update(self.wx_gui.filterTeamPlayers(self.wx_gui.visual_idToPlayers))
        else:
            self.wx_gui.dominant_region.remove()
    ...
    def activate(self):
        ...
        self.wx_gui.Bind(wx.EVT_MENU, self.on_dominant_region, self.layouts.dominant_region)
