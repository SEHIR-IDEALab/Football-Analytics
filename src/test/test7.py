...
class Visualization(object):
    def __init__(self, coordinateData_byTme, eventData_byTime, teamNames, minMaxOfHalf, id_explanation):
        ...
        self.text_toDisplayPasses = Tk.Text(self.frame, width=100)
        yscrollbar = Tk.Scrollbar(self.frame, orient=VERTICAL, command=self.text_toDisplayPasses.yview)
        ...
        scale = Tk.Scale(self.frame,orient=Tk.HORIZONTAL, length=800,  sliderlength=10, showvalue=0,
                 from_=0, to=90, resolution=(1.0/300.0), variable=self.scaleVariable, tickinterval=15)
        displayChosenPoint = Tk.Label(master=self.frame, textvariable=self.effec_withCompVariable_forChosenPoint, justify=LEFT)
        rb_pass = Tk.Radiobutton(self.frame, text="Define Pass", variable=rb_var, value=1,
                                 command=self.rb_define_pass)
        rb_drag = Tk.Radiobutton(self.frame, text= "Draggable Objects", variable=rb_var,value=2,
                                 command=self.rb_drag_objects)
        heatmapType_options = Tk.OptionMenu(self.frame, self.heatmapTypeVariable, "-----", "defence position taking",
                                            "position of target of pass", "position of source of pass")
        ...
        componentsOfEffectiveness_options = Tk.OptionMenu(self.frame, self.componentsOfEffectivenessVariable,
                            "effectiveness", "gain", "pass advantage", "goal chance", "overall risk")
        button_scaleDraw = Tk.Button(master=self.frame, text='Draw', command=self.scaleDraw)
        button_play = Tk.Button(master=self.frame, text='Play', command=self.play)
        button_pause = Tk.Button(master=self.frame, text='Pause', command=self.pause)
        self.play_options = Tk.OptionMenu(self.frame, self.playVariable, "slow", "normal", "fast")
        self.skipPlay_options = Tk.OptionMenu(self.frame, self.skipPlayVariable, "0", "1", "2", "3", "4")
        self.button_quit = Tk.Button(self.frame, text="Quit", command=self.quit)
        self.label = Tk.Label(master=self.frame, textvariable = self.currentTimeVariable)


        self.frame.grid(row=0, column=0, rowspan=10, columnspan=5)
        self.text_toDisplayPasses.grid(row=0, column=0)
        yscrollbar.grid()
        displayChosenPoint.grid(row=7, column=0, rowspan=3, columnspan=3)
        scale.grid(row=8, column=3, columnspan=15)
        rb_pass.grid(row=9, column=10)
        rb_drag.grid(row=8, column=10)
        #heatmapType_options.grid(row=17, column=17)
        #resolutionLevel_options.grid(row=17, column=16)
        #button_pause.grid(row=17, column=12)
        #self.play_options.grid(row=17, column=11)
        #self.skipPlay_options.grid(row=17, column=10)
        #self.button_quit.grid(row=17, column=9)
        #self.label.grid(row=17, column=8)
        #componentsOfEffectiveness_options.grid(row=17, column=15)
        #button_scaleDraw.grid(row=17, column=14)
        #button_play.grid(row=17, column=13)
        ...
        # self.canvas.show()
        # self.canvas.get_tk_widget().grid(row=1,column=7,columnspan=17, rowspan=15)
        #
        # self.toolbar = NavigationToolbar2TkAgg(self.canvas, self.master)
        # self.toolbar.update()
        # self.canvas._tkcanvas.grid(row=24,column=24)
        #
